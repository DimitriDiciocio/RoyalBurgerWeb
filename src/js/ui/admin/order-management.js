...
(function initOrderManagement() {
    ...
    function cleanupPhoneCache() {
        ...
        if (cacheKeys.length > MAX_PHONE_CACHE_SIZE) {
            ...
            const keysToRemove = cacheKeys.slice(0, Math.floor(cacheKeys.length * 0.2));
    ...
    function calculateTimeEstimate(order) {
        ...
        const diffMinutes = (diffMs > 0 && diffMs < 86400000) ? Math.floor(diffMs / 60000) : 0; // Máximo 24h
    ...
    function getActionButtonText(status, isPickup = false) {
        ...
        const normalizedStatus = (status === 'in_progress' && isPickup) ? 'ready' : status;
    ...
    async function fetchUserPhone(userId) {
        ...
        try {
            ...
            if (isDevelopment()) {
                console.warn(`Erro ao buscar telefone do usuário ${userId}:`, error.message);
            }
    ...
    function findPhoneInOrderObject(order) {
        ...
        if (order.customer && typeof order.customer === 'object') {
            ...
            const phone = customer.phone || customer.telefone || customer.mobile_phone || customer.mobile;
        ...
        if (order.user && typeof order.user === 'object') {
            ...
            const phone = user.phone || user.telefone || user.mobile_phone || user.mobile;
            if (phone) return phone;
        }
    ...
    function renderOrders() {
        ...
        const ordersHtml = state.filteredOrders.map(order => {
            ...
            const customerName = escapeHTML(
                order.customer_name || 
                (order.customer && typeof order.customer === 'object' ? (order.customer.full_name || order.customer.name) : '') ||
            ...
            const itemsHtml = items.length > 0 ? items.map(item => {
                ...
                if (extras && extras.length > 0) {
                    const extrasItems = extras.map(extra => {
                        ...
                        const precoFormatado = preco > 0 ? ` <span class="extra-price">+R$ ${escapeHTML(preco.toFixed(2).replace('.', ','))}</span>` : '';
                ...
                if (baseMods && baseMods.length > 0) {
                    ...
                    const baseModsItems = baseMods.map(bm => {
                        ...
                        const precoFormatado = (preco > 0 && isPositive) ? ` <span class="base-mod-price">+R$ ${escapeHTML(preco.toFixed(2).replace('.', ','))}</span>` : '';
