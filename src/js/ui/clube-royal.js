...
(function initClubeRoyalPage() {
    ...
    try {
      ...
      state.historico = limitedHistory
        .map((item) => {
          ...
          const isBoasVindas =
            item.reason && item.reason.toLowerCase().includes("boas-vindas");
          ...
          if (descricao.length > VALIDATION_LIMITS.MAX_DESCRIPTION_LENGTH) {
            descricao =
              descricao.substring(0, VALIDATION_LIMITS.MAX_DESCRIPTION_LENGTH) +
              "...";
          }

          if (isBoasVindas) {
            descricao =
              "Bem-vindo ao Clube Royal! Você ganhou pontos de boas-vindas";
          } else if (isGanho && isOrderRelated) {
            descricao = `Você fez uma compra e ganhou ${Math.abs(
              pontos
            )} pontos`;
          } else if (!isGanho) {
          ...
          const validoAte = item.expiration_date
  ...
  function criarElementoHistorico(item) {
    ...
    const pontosTexto =
      item.pontos > 0 ? `+${item.pontos} pontos` : `${item.pontos} pontos`;
    ...
    div.innerHTML = `
            ...
            <div class="texto">
                ...
                <div class="descricao">
                    ...
                    <p class="escuro" style="color: ${tipoCor};">${escapeHTML(
      pontosTexto
    )}</p>
                    ${
                      item.validoAte
                        ? `<p class="claro">Válido até ${formatDate(
                            item.validoAte
                          )}</p>`
                        : ""
                    }
  ...
  async function atualizarExplicacoes() {
    try {
      ...
      const descontoUmReal = (pontosParaUmReal * redemptionRate).toLocaleString(
        "pt-BR",
        {
          style: "currency",
          currency: "BRL",
      ...
      const descricao2 = document.querySelector(
        "#secao-explicacao .informa > div:nth-child(2) .descricao"
      );
      if (descricao2) {
      ...
      const descricao4 = document.querySelector(
        "#secao-explicacao .informa > div:nth-child(4) .descricao"
      );
      if (descricao4) {
